...
poisot1 <- read_csv(paste0(GithubDir, "PoisotKnn1Bat-June.csv"), col_names = FALSE)
poisot2 <- read_csv(paste0(GithubDir, "PoisotKnn2Bat-June.csv"), col_names = FALSE)
poisot3 <- read_csv(paste0(GithubDir, "PoisotLfBat-June.csv"), col_names = FALSE)
...
poisot1 <- read_csv(paste0(GithubDir, "PoisotKnn1Mammal-June.csv"), col_names = FALSE)
poisot2 <- read_csv(paste0(GithubDir, "PoisotKnn2Mammal-June.csv"), col_names = FALSE)
poisot3 <- read_csv(paste0(GithubDir, "PoisotLfMammal-June.csv"), col_names = FALSE)
...
ModelList %>%
  map(~.x %>% mutate_at(vars(contains("Po")), ~str_remove(.x, "X") %>% as.numeric) %>% 
        mutate_at("Sp", ~str_replace_all(.x, "[.]", " "))) %>% 
  map(~.x %>% 
        mutate_at("Sp", function(a) a %>% 
                    str_trim %>% 
                    str_replace("_", " ")) %>%
...
dir_create("Cleaned Files_2020")

BatModels_OS %>% write.csv("Cleaned Files_2020/BatModels_OS.csv", row.names = F)
BatModels_IS %>% write.csv("Cleaned Files_2020/BatModels_IS.csv", row.names = F)
NonBatModels_OS %>% write.csv("Cleaned Files_2020/NonBatModels_OS.csv", row.names = F)
NonBatModels_IS %>% write.csv("Cleaned Files_2020/NonBatModels_IS.csv", row.names = F)
